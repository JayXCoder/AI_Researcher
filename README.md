...
## Functional diagram (NotebookLM)

End-to-end flow of the system:

![Functional diagram generated by NotebookLM](assets/functional-diagram.png)

Same flow as Mermaid (for repos that render it):

```mermaid
flowchart TB
    User([User]) --> |"question"| UI[Flutter Web UI]
    UI --> |POST /query| Gateway[Backend API]
    Gateway --> Planner[Planner Agent]
    Planner --> |steps| Gateway
    Gateway --> Search[Search Agent]
    Search --> |"API"| Web[Custom Search / SerpAPI]
    Web --> |results| Search
    Gateway --> Retriever[Retriever Agent]
    Retriever --> |embeddings| Vertex[Vertex AI]
    Vertex --> |chunks| Retriever
    Search --> Verifier[Verifier Agent]
    Retriever --> Verifier
    Verifier --> |verified sources| Answer[Answer Agent]
    Answer --> |generate| Vertex
    Vertex --> |draft| Answer
    Answer --> Reflection[Reflection Agent]
    Reflection --> |improve| Vertex
    Vertex --> |final| Reflection
    Reflection --> Gateway
    Gateway --> |answer + citations| UI
    UI --> User
```
